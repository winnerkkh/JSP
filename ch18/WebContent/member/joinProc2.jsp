...
function chk(){
	if(!document.frm.ok.checked){
		alert("개인정보 취급방침 에 동의하셔야 회원가입이 됩니다.");
		return false;
	}
	return true;
}
...
<body>
<h2>개인정보 취급방침</h2>
<form name="frm" action="subscribe.jsp" onsubmit="return chk()">
<pre><!-- 입력한 내용 대로 출력 -->
<hr>
제1조(목적)

본 회원약관은 중앙정보기술인재개발원/중앙정보처리학원(이하 ‘갑’라 한다)가 운영하는 홈페이지 서비스(이하 ‘서비스‘라 한다)를 이용함에 있어 관리자와 이용자(이하 ‘회원’라 한다)의 권리, 의무 및 책임사항을 규정함을 목적으로 한다.
...
4.이메일(EMAIL) : 회원 식별과 회원의 서비스 이용을 위하여 회원이 선정한 인터넷 메일형식의 주소
...
2.가입희망 회원은 반드시 자신의 이메일로 이용신청을 하여야 하며, 1개의 ID만 신청을 할 수 있습니다.
...
1. ‘갑’은 타인의 명의나 이메일 주소를 도용하여 회원가입신청을 할 경우 회원가입신청을 거절할 수 있다.
...
<hr><br>
동의여부: <input type="checkbox" name=ok><br>
